...
public class GitBlitUrlsConfig {
  ...
  private String[] downloadSchemes;
  ...
  public GitBlitUrlsConfig(Config config) {
    ...
    downloadSchemes = config.getStringList("download", null, "scheme");
  }
  ...
  public String getGitHttpUrl() throws UnknownHostException {
    ...
    if (httpListenUrl == null) {
      ...
      if (downloadSchemes.length != 0){
       	if (!Arrays.asList(downloadSchemes).contains("http")){//downloadScheme is provided, but it doesnt include http
          return "";
	  	}
	  }
