...
public class GitBlitTopMenu implements TopMenu {
  ...
  public GitBlitTopMenu(@PluginName String pluginName,
      ...
      Provider<CurrentUser> userProvider, PluginConfigFactory cfgProvider) {
    this.userProvider = userProvider;

    String gitBlitBaseUrl =
        pluginUrl.endsWith("/") ? pluginUrl : pluginUrl + '/';
    ...
    // Actually, I'd like to give the project "browse" link only if the user has
    // the right to see the contents of the repository. But how would
    // I know in getEntries() below which is the "current" project? Since I
    // don't know how to do that, we show that link always, based on the
    // assumption that if a user can see a project at all, he can also see its
    // contents. If not, gitblit will tell him so...
    extraProjectEntries =
